...
SoftwareSerial cairsensSerial(16, 15); //RX, TX
...
static CairsensUART cairsens(&cairsensSerial);
/** Sample time in ms */
static const unsigned long NOX_SAMPLE_TIME = 100;
static unsigned long nox_starttime = 0;
...
void loop() {
  ...
  if ((millis() - nox_starttime) > NOX_SAMPLE_TIME)
  {
    uint8_t nox_val = 0;
    if(cairsens.getInstantValue(nox_val) == CairsensUART::NO_ERROR)
    {
      Serial.print("Current NOX value = ");
      Serial.print(nox_val/10.0);
      Serial.println("ppm");
    }
    else
    {
      Serial.println("Could not get Cairsens NOX value");
    }
    nox_starttime = 0;
  }
}
